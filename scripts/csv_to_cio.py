...
custom_headers = [
    headers[header_indices[normalized_col]] for normalized_col in custom_headers
]
...
def lead_from_row(row):
    row = {
        column_name: column_value.strip() for column_name, column_value in row.items()
    ...
    lead = {"contacts": [], "custom": {}}
    if "company" in row:
        lead["name"] = row["company"]
    ...
    phones = []
    if "phone" in row:
        phones.append({"phone": row["phone"], "type": "direct"})
    ...
    emails = []
    if "email" in row:
        emails.append({"email": row["email"], "type": "direct"})
    if len(emails):
        contact["emails"] = emails

    if "date_created" in row:
